...
_B1='file_path = '
_B0='select2_update'
_A_='Access From Menu cannot empty!'
_Az='/pages/detail/'
...
class IndexView(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		elif self.site_id==-2:return redirect(_Av)
		elif self.site_id==-3 or self.site_id==-31:return redirect(reverse_lazy('user_init_agency'))
		elif self.site_id==-4 or self.site_id==-41:return redirect(reverse_lazy('user_init_service',kwargs={_Aw:get_agency_from(request)}))
	...
	def get_context_data(self,*args,**kwargs):
		context=super(IndexView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_A8);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);user=User.objects.get(id=self.request.user.id);agency=user.agency.all();context[_k]=agency;service=[]
...
def user_init_agency(request):
	...
	else:return redirect(_Av)
	if request.method==_I:
		name=request.POST.get('agent-name');agency,created=Agency.objects.language(_B).get_or_create(name=name,defaults={'is_default':_b,_A9:'',_AA:''});agency.set_current_language(_L);agency.address='';agency.notes='';agency.save();print(f"DATA: {agency} - {created}")
...
def user_init_agency_ajax(request,agency_id):
	...
	return JsonResponse({_AZ:_b},safe=_A)
def user_init_service_ajax(request,agency_id,service_id):
	...
	if mfound:
		...
		return JsonResponse({_AZ:_b},safe=_A)
	return JsonResponse({_AZ:_A},safe=_A)
@transaction.atomic
def user_init_service(request,agency_id):
	context={};template='backend/smart-admin-2/user_initialize_service.html';context['service_opt']=OptServiceType.choices;service_existing=Service.objects.filter(agency_id=agency_id).order_by('kind');context[_p]=service_existing;context[_Aw]=agency_id
	if request.method==_I:
...
def tags_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_AB]=Truncator(i.name_id).chars(50);res[_AC]=Truncator(i.name).chars(50);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
...
def tags_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if tmp:messages.info(request,mMsgBox.get(_Ax));context[_G]=TagsForm()
...
class LogoView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(LogoView,self).get_context_data(*(args),**kwargs);context[_AS]=_b;active_page=get_translated_active_page(_q);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def logo_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_O));messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_q))
...
def fasilities_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_l]=Truncator(i.content_id).chars(50);res[_m]=Truncator(i.content).chars(50);res[_j]=i.file_path;res[_AD]=i.is_header_text;res[_Aa]=i.order_item;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def fasilities_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_n];obj.status=request.POST.get(_N);obj.save()
			if photo.is_valid():
				if request.POST.get(_O):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_O))
...
def offers_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():
			post=Offers.objects.language(_B).create(title=request.POST.get(_F),content=request.POST.get(_K),is_header_text=form.cleaned_data[_n],status=request.POST.get(_N));post.set_current_language(_L);post.title=request.POST.get(_F);post.content=request.POST.get(_K);post.is_header_text=form.cleaned_data[_n];post.save()
			if request.POST.get(_O):Photo.objects.create(content_object=post,file_path=request.POST.get(_O))
			messages.info(request,mMsgBox.get(_a,request.POST.get(_F)));return redirect(reverse_lazy(_t))
	else:messages.info(request,mMsgBox.get(_U));context[_G]=OffersForm();context[_M]=PhotoForm()
	return render(request,template,context)
...
def news_ajax(request):
	site_id=get_site_id(request);obj=News();obj.set_current_language(_L);subquery1=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(NewsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_u).values(_i)[:1]);lang=obj.get_current_language();obj2=News.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_l]=Truncator(i.content_id).chars(50);res[_m]=Truncator(i.content).chars(50);res[_j]=i.file_path;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def article_ajax(request):
	site_id=get_site_id(request);obj=Article();obj.set_current_language(_L);subquery1=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(ArticleTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_v).values(_i)[:1]);lang=obj.get_current_language();obj2=Article.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
...
def article_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.categories_id=request.POST.get(_f);obj.status=request.POST.get(_N);obj.is_header_text=form.cleaned_data[_n];obj.save();obj.tags.clear();save_tags(request.POST.getlist(_e),obj)
			if photo.is_valid():
				if request.POST.get(_O):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_O))
...
def events_ajax(request):
	site_id=get_site_id(request);obj=Events();obj.set_current_language(_L);subquery1=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(EventsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_w).values(_i)[:1]);lang=obj.get_current_language();obj2=Events.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_l]=Truncator(i.content_id).chars(50);res[_m]=Truncator(i.content).chars(50);res[_j]=i.file_path;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
class SlideShowView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(SlideShowView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ab);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def slideshow_ajax(request):
	site_id=get_site_id(request);lst=[];obj=SlideShow();obj.set_current_language(_L);subquery1=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery3=Subquery(SlideShowTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AT));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='slideshow').values(_i)[:1]);lang=obj.get_current_language();obj2=SlideShow.objects.language(lang).filter(site_id=site_id).distinct().annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(sub_title_id=subquery3).annotate(file_path=subquery_foto)
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res['Sub Title (id)']=Truncator(i.sub_title_id).chars(50);res['Sub Title (en)']=Truncator(i.sub_title).chars(50);res[_l]=Truncator(i.content_id).chars(50);res[_m]=Truncator(i.content).chars(50);res[_j]=i.file_path;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def slideshow_create(request):
	context={};context[_J]=_AE;active_page=get_translated_active_page(_Ab);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
		form=SlideShowForm(request.POST);photo=PhotoForm(request.POST)
		if form.is_valid():post=SlideShow.objects.language(_B).create(title=request.POST.get(_F),sub_title=request.POST.get(_AT),content=request.POST.get(_K),status=request.POST.get(_N));post.set_current_language(_L);post.title=request.POST.get(_F);sub_title=request.POST.get(_AT);post.content=request.POST.get(_K);post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_O));messages.info(request,mMsgBox.get(_a,request.POST.get(_F)));return redirect(reverse_lazy(_AE))
		else:print(_A4);context[_G]=SlideShowForm();context[_M]=PhotoForm()
	else:messages.info(request,mMsgBox.get(_U));context[_G]=SlideShowForm();context[_M]=PhotoForm()
	return render(request,template,context)
def slideshow_update(request,uuid):
	context={};context[_J]=_AE;active_page=get_translated_active_page(_Ab);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_V;data=SlideShow.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);post_photo=post.photo.first()
	if request.method==_I:
		form=SlideShowForm(request.POST,instance=post);photo=PhotoForm(request.POST,instance=post_photo)
		if form.is_valid():
			lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.sub_title=request.POST.get(_AT);obj.content=request.POST.get(_K);obj.status=request.POST.get(_N);obj.save()
			...
			messages.info(request,mMsgBox.get(_W,request.POST.get(_F)));return redirect(reverse_lazy(_AE))
	else:
		messages.info(request,mMsgBox.get(_X));context[_G]=SlideShowForm(instance=post)
		if post_photo:context[_M]=PhotoForm(instance=post_photo)
		else:context[_M]=PhotoForm()
	return render(request,template,context)
def slideshow_delete(request,uuid):context={};site_id=get_site_id(request);data=SlideShow.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AE))
class DailyAlertView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(DailyAlertView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ac);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def dailyalert_ajax(request):
	site_id=get_site_id(request);lst=[];obj=DailyAlert();obj.set_current_language(_L);subquery1=Subquery(DailyAlertTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_A6));lang=obj.get_current_language();obj2=DailyAlert.objects.language(lang).filter(site_id=site_id).annotate(alert_id=subquery1)
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res['Alert (id)']=Truncator(i.alert_id).chars(50);res['Alert (en)']=Truncator(i.alert).chars(50);res[_AF]=Truncator(i.link).chars(50);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def dailyalert_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=DailyAlert.objects.language(_B).create(alert=request.POST.get(_A6),link=request.POST.get(_x),status=request.POST.get(_N));post.set_current_language(_L);post.alert=request.POST.get(_A6);post.save();messages.info(request,mMsgBox.get(_a,request.POST.get(_A6)));return redirect(reverse_lazy(_AG))
		else:print(_A4);context[_G]=DailyAlertForm()
...
def dailyalert_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.alert=request.POST.get(_A6);obj.link=request.POST.get(_x);obj.status=request.POST.get(_N);obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_A6)));return redirect(reverse_lazy(_AG))
	else:messages.info(request,mMsgBox.get(_X));context[_G]=DailyAlertForm(instance=post)
	return render(request,template,context)
def dailyalert_delete(request,uuid):context={};site_id=get_site_id(request);data=DailyAlert.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.alert;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AG))
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(GreetingView,self).get_context_data(*(args),**kwargs);context[_AS]=_b;active_page=get_translated_active_page(_y);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def greeting_ajax(request):
	site_id=get_site_id(request);obj=Greeting();obj.set_current_language(_L);subquery1=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(GreetingTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_y).values(_i)[:1]);lang=obj.get_current_language();obj2=Greeting.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_l]=Truncator(i.content_id).chars(50);res[_m]=Truncator(i.content).chars(50);res[_j]=i.file_path;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def greeting_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.name=request.POST.get(_H);obj.designation=request.POST.get(_Ad);obj.status=request.POST.get(_N);obj.save()
			if photo.is_valid():
				if request.POST.get(_O):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_O))
...
def pages_ajax(request):
	site_id=get_site_id(request);obj=Pages();obj.set_current_language(_L);subquery1=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(PagesTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model=_z).values(_i)[:1]);lang=obj.get_current_language();obj2=Pages.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_l]=Truncator(i.content_id).chars(50);res[_m]=Truncator(i.content).chars(50);res[_j]=i.file_path;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def pages_create(request):
	context={};context[_J]=_z;active_page=get_translated_active_page(_z);context[_E]=active_page;context[_AU]=_Ay;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
		...
		if form.is_valid():
			...
			if menu_id:
				...
				if not menu_name_already_used:post=Pages.objects.language(_B).create(title=request.POST.get(_F),content=request.POST.get(_K),menu_id=menu_id,status=request.POST.get(_N));post.set_current_language(_L);post.title=request.POST.get(_F);post.content=request.POST.get(_K);post.save();save_tags(request.POST.getlist(_e),post);Photo.objects.create(content_object=post,file_path=request.POST.get(_O));obj=Menu.objects.get(id=menu_id);obj.link=_Az+post.slug;obj.save();messages.info(request,mMsgBox.get(_a,request.POST.get(_F)));return redirect(reverse_lazy(_z))
				else:messages.info(request,mMsgBox.get(_AV,'Menu already used by '+menu_name_already_used));context[_G]=PagesForm(request.POST);context[_M]=PhotoForm(request.POST)
			else:messages.info(request,mMsgBox.get(_AV,_A_));context[_G]=PagesForm(request.POST);context[_M]=PhotoForm(request.POST)
	else:messages.info(request,mMsgBox.get(_U));context[_G]=PagesForm();context[_M]=PhotoForm()
	return render(request,template,context)
def pages_update(request,uuid):
	context={};context[_J]=_z;active_page=get_translated_active_page(_z);context[_E]=active_page;context[_AU]=_Ay;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_V;data=Pages.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);post_photo=post.photo.all()
	if post_photo:post_photo=post_photo.first()
	select2_update={_B:post.menu_id,'text':post.menu.name if post.menu else _C};context[_B0]=select2_update
	if request.method==_I:
		...
		if form.is_valid():
			...
			if menu_id:
				...
				if photo.is_valid():
					if request.POST.get(_O):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_O))
				obj=Menu.objects.get(id=menu_id);obj.link=_Az+post.slug;obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_F)));return redirect(reverse_lazy(_z))
			else:
				messages.info(request,mMsgBox.get(_AV,_A_));context[_G]=PagesForm(request.POST,instance=post)
...
class SocialMediaView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(SocialMediaView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ae);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def socialmedia_ajax(request):
	site_id=get_site_id(request);obj2=SocialMedia.objects.filter(site_id=site_id);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res['Kind']=i.get_kind_display();res[_AF]=Truncator(i.link).chars(70);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
class HowItWorksView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(HowItWorksView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Af);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def howitworks_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(70);res[_h]=Truncator(i.title).chars(70);res[_l]=Truncator(i.content_id).chars(70);res[_m]=Truncator(i.content).chars(70);res['Icon']=i.icon;res[_AD]=i.is_header_text;res[_Aa]=i.order_item;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def howitworks_create(request):
	context={};context[_J]=_AJ;active_page=get_translated_active_page(_Af);print(_E,active_page);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
...
class PhotoGalleryView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(PhotoGalleryView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ag);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def photogallery_ajax(request):
	site_id=get_site_id(request);lst=[];obj=PhotoGallery();obj.set_current_language(_L);subquery1=Subquery(PhotoGalleryTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(PhotoGalleryTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_K));subquery_foto=Subquery(Photo.objects.filter(object_id=OuterRef(_B),content_type__model='photogallery').values(_i)[:1]);lang=obj.get_current_language();obj2=PhotoGallery.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery1).annotate(content_id=subquery2).annotate(file_path=subquery_foto)
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_l]=Truncator(i.content_id).chars(100);res[_m]=Truncator(i.content).chars(100);res[_j]=i.file_path;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def photogallery_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=PhotoGallery.objects.language(_B).create(title=request.POST.get(_F),status=request.POST.get(_N),content=request.POST.get(_K));post.set_current_language(_L);post.title=request.POST.get(_F);post.content=request.POST.get(_K);post.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_O));messages.info(request,mMsgBox.get(_a,request.POST.get(_F)));return redirect(reverse_lazy(_AK))
		else:print(_A4);context[_G]=PhotoGalleryForm();context[_M]=PhotoForm()
...
def photogallery_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.status=request.POST.get(_N);obj.content=request.POST.get(_K);obj.save()
			if photo.is_valid():
				if request.POST.get(_O):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_O))
	...
	return render(request,template,context)
def photogallery_delete(request,uuid):context={};site_id=get_site_id(request);data=PhotoGallery.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AK))
...
class VideoGalleryView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(VideoGalleryView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ah);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def videogallery_create(request):
	context={};context[_J]=_AL;active_page=get_translated_active_page(_Ah);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
...
class RelatedLinkView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(RelatedLinkView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ai);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def relatedlink_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_AB]=Truncator(i.name_id).chars(50);res[_AC]=Truncator(i.name).chars(50);res[_AF]=Truncator(i.link).chars(70);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def relatedlink_create(request):
	context={};context[_J]=_AM;active_page=get_translated_active_page(_Ai);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
		form=RelatedLinkForm(request.POST)
		if form.is_valid():post=RelatedLink.objects.language(_B).create(name=request.POST.get(_H),link=request.POST.get(_x),status=request.POST.get(_N));post.set_current_language(_L);post.name=request.POST.get(_H);post.save();messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_AM))
		else:print(_A4);context[_G]=RelatedLinkForm()
	else:messages.info(request,mMsgBox.get(_U));context[_G]=RelatedLinkForm()
	return render(request,template,context)
def relatedlink_update(request,uuid):
	context={};context[_J]=_AM;active_page=get_translated_active_page(_Ai);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_V;data=RelatedLink.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
		form=RelatedLinkForm(request.POST,instance=post)
		if form.is_valid():lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.name=request.POST.get(_H);obj.link=request.POST.get(_x);obj.status=request.POST.get(_N);obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_H)));return redirect(reverse_lazy(_AM))
	else:messages.info(request,mMsgBox.get(_X));context[_G]=RelatedLinkForm(instance=post)
	return render(request,template,context)
def relatedlink_delete(request,uuid):context={};site_id=get_site_id(request);data=RelatedLink.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AM))
class DocumentView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(DocumentView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_A2);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def document_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_AB]=Truncator(i.name_id).chars(20);res[_AC]=Truncator(i.name).chars(20);res['content (id)']=Truncator(i.content_id).chars(30);res['content (en)']=Truncator(i.content).chars(30);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def document_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():print('categories = ',request.POST.get(_f));print('file = ',request.FILES.get(_AX));post=Document.objects.language(_B).create(name=request.POST.get(_H),content=request.POST.get(_K),file_path_doc=request.FILES.get(_AX),categories_id=request.POST.get(_f),status=request.POST.get(_N));post.set_current_language(_L);post.name=request.POST.get(_H);post.content=request.POST.get(_K);post.save();print(_B1,post.file_path_doc.path);post.size=os.stat(post.file_path_doc.path).st_size;post.save();messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_A2))
...
def document_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			...
			if request.FILES.get(_AX):obj.file_path_doc=request.FILES.get(_AX)
			obj.status=request.POST.get(_N);obj.categories_id=request.POST.get(_f);obj.save();print(_B1,obj.file_path_doc.path);obj.size=os.stat(obj.file_path_doc.path).st_size;obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_H)));return redirect(reverse_lazy(_A2))
	else:messages.info(request,mMsgBox.get(_X));context[_G]=DocumentForm(instance=post)
	return render(request,template,context)
def document_delete(request,uuid):context={};site_id=get_site_id(request);data=Document.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_A2))
	...
	if group_id:
		...
		if menu:
			...
			for i in obj2:
				...
				res={};res[_P]=_C;res[_Q]=i[_Q];res[_R]=_C;res[A+lang+')']=Truncator(i[_H]).chars(50);res[A+lang2+')']=Menu.objects.language(lang2).get(pk=i[_B]).name;res['Tree']=tmp+Truncator(i[_H]).chars(70);res[_AF]=Truncator(i[_x]).chars(70);res['Order']=i[_Aj];res['Icon']=Truncator(i[_P]).chars(50);res['Visibled']=Truncator(i[_Ak]).chars(50);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def menu_create(request):
	context={};context[_J]=_D;active_page=get_translated_active_page(_D);context[_E]=active_page;context[_AU]=_B2;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);group_id=get_menu_group(site_id);menu_group=MenuGroup.objects.get(id=group_id);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
		form=MenuForm(request.POST)
		if form.is_valid():form_clean=form.cleaned_data;print('select2_menu = ',request.POST.get(_AH));post=Menu.objects.language(_B).create(name=form_clean[_H],parent_id=request.POST.get(_AH),link=form_clean[_x],order_menu=form_clean[_Aj],icon=form_clean[_P],is_visibled=form_clean[_Ak],is_external=form_clean[_B3]);post.menu_group.add(menu_group);post.set_current_language(_L);post.name=form_clean[_H];post.save();messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_D))
		else:print(_A4);context[_G]=MenuForm()
	else:messages.info(request,mMsgBox.get(_U));context[_G]=MenuForm()
	return render(request,template,context)
def menu_update(request,uuid):
	context={};context[_J]=_D;active_page=get_translated_active_page(_D);context[_E]=active_page;context[_AU]=_B2;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_V;data=Menu.objects.filter(uuid=uuid);post=get_object_or_404(data);select2_update={_B:post.parent_id,'text':post.parent.name if post.parent else _C};context[_B0]=select2_update
	if request.method==_I:
		form=MenuForm(request.POST,instance=post)
		if form.is_valid():form_clean=form.cleaned_data;lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.name=form_clean[_H];obj.parent_id=request.POST.get(_AH);obj.link=form_clean[_x];obj.order_menu=form_clean[_Aj];obj.icon=form_clean[_P];obj.is_visibled=form_clean[_Ak];obj.is_external=form_clean[_B3];obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_H)));return redirect(reverse_lazy(_D))
...
class AgencyView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(AgencyView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_k);context[_E]=active_page;context[_AS]=_b;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def agency_ajax(request):
	site_id=get_site_id(request);obj=Agency();obj.set_current_language(_L);service=Service.objects.filter(site_id=site_id).values_list(_k,flat=_b);subquery1=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_A9));subquery2=Subquery(AgencyTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_AA));lang=obj.get_current_language();obj2=Agency.objects.language(lang).filter(id=service[0]).annotate(address_id=subquery1).annotate(notes_id=subquery2);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_A5]=Truncator(i.name).chars(50);res['Address (id)']=Truncator(i.address_id).chars(50);res['Address (en)']=Truncator(i.address).chars(50);res[_B4]=Truncator(i.notes_id).chars(50);res[_B5]=Truncator(i.notes).chars(50);res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def product_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_AB]=Truncator(i.name_id).chars(50);res[_AC]=Truncator(i.name).chars(50);res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_l]=Truncator(i.content_id).chars(50);res[_m]=Truncator(i.content).chars(50);res[_AD]=i.is_header_text;res[_j]=i.file_path;res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
...
def product_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.name=request.POST.get(_H);obj.title=request.POST.get(_F);obj.content=request.POST.get(_K);obj.is_header_text=form.cleaned_data[_n];obj.status=request.POST.get(_N);obj.save();print('photo file path',request.POST.get(_O))
			if request.POST.get(_O):post.photo.clear();Photo.objects.create(content_object=post,file_path=request.POST.get(_O))
...
class TemplateOwnerView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(TemplateOwnerView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Al);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def templateowner_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=TemplateOwner.objects.create(name=request.POST.get(_H));messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_AN))
...
def templateowner_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_H);obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_H)));return redirect(reverse_lazy(_AN))
	else:messages.info(request,mMsgBox.get(_X));context[_G]=TemplateOwnerForm(instance=post)
	return render(request,template,context)
def templateowner_delete(request,uuid):context={};site_id=get_site_id(request);data=TemplateOwner.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AN))
class TemplateView_(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(TemplateView_,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Am);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def template_create(request):
	context={};context[_J]=_AO;active_page=get_translated_active_page(_Am);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
		form=TemplateForm(request.POST)
		if form.is_valid():tmp=request.POST.get(_An);post=Template.objects.create(name=request.POST.get(_H),rel_path=request.POST.get('rel_path'),is_frontend=bool(request.POST.get(_B7)),template_owner_id=tmp if tmp else _C);messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_AO))
...
def template_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():
			obj=data.get();tmp=request.POST.get(_An);obj.name=request.POST.get(_H);obj.rel_path=request.POST.get('rel_path');obj.is_frontend=bool(request.POST.get(_B7))
			if tmp:obj.template_owner_id=request.POST.get(_An)
			obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_H)));return redirect(reverse_lazy(_AO))
	else:messages.info(request,mMsgBox.get(_X));context[_G]=TemplateForm(instance=post)
	return render(request,template,context)
def template_delete(request,uuid):context={};site_id=get_site_id(request);data=Template.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AO))
class ModelListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(ModelListView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ao);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def modellist_ajax(request):
	site_id=get_site_id(request);obj2=ModelList.objects.all();lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_A5]=Truncator(i.name).chars(50);res['Description']=Truncator(i.description).chars(50);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def modellist_create(request):
	context={};context[_J]=_AP;active_page=get_translated_active_page(_Ao);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
		form=ModelListForm(request.POST)
		if form.is_valid():post=ModelList.objects.create(name=request.POST.get(_H),description=request.POST.get(_A7),status=request.POST.get(_N));messages.info(request,mMsgBox.get(_a,request.POST.get(_H)));return redirect(reverse_lazy(_AP))
...
def modellist_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_H);obj.description=request.POST.get(_A7);obj.status=request.POST.get(_N);obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_H)));return redirect(reverse_lazy(_AP))
	else:messages.info(request,mMsgBox.get(_X));context[_G]=ModelListForm(instance=post)
	return render(request,template,context)
def modellist_delete(request,uuid):context={};site_id=get_site_id(request);data=ModelList.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AP))
...
def banner_ajax(request):
	...
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res['Priority']=i.get_priority_display();res[_j]=i.file_path;res[_AF]=Truncator(i.link).chars(70);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def banner_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=form.save();Photo.objects.create(content_object=post,file_path=request.POST.get(_O));messages.info(request,mMsgBox.get(_a,request.POST.get('position')));return redirect(reverse_lazy(_A1))
...
class LocationView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(LocationView,self).get_context_data(*(args),**kwargs);context[_AS]=_b;active_page=get_translated_active_page(_o);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def location_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():post=Location.objects.language(_B).create(title=request.POST.get(_F),embed=request.POST.get(_AW),status=request.POST.get(_N));post.set_current_language(_L);post.title=request.POST.get(_F);post.save();messages.info(request,mMsgBox.get(_a,request.POST.get(_F)));return redirect(reverse_lazy(_o))
		else:print(_A4);context[_G]=LocationForm()
...
def location_update(request,uuid):
	...
	if request.method==_I:
		...
		if form.is_valid():lang=request.POST.get(_d);obj=data.get();obj.set_current_language(lang);obj.title=request.POST.get(_F);obj.embed=request.POST.get(_AW);obj.status=request.POST.get(_N);obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_F)));return redirect(reverse_lazy(_o))
...
class WhyUsView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(WhyUsView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Ap);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
def whyus_ajax(request):
	site_id=get_site_id(request);obj=WhyUs();obj.set_current_language(_L);subquery=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_F));subquery2=Subquery(WhyUsTranslation.objects.filter(master_id=OuterRef(_B),language_code=_B).values(_A7));lang=obj.get_current_language();obj2=WhyUs.objects.language(lang).filter(site_id=site_id).annotate(title_id=subquery).annotate(description_id=subquery2);lst=[]
	for i in obj2:res={};res[_P]=_C;res[_Q]=i.uuid;res[_R]=i.updated_at;res[_g]=Truncator(i.title_id).chars(50);res[_h]=Truncator(i.title).chars(50);res[_B4]=Truncator(i.description_id).chars(100);res[_B5]=Truncator(i.description).chars(100);res['Icon']=Truncator(i.icon).chars(20);res[_c]=i.get_status_display();res[_Z]=get_natural_datetime(i.updated_at);res[_S]=_C;lst.append(res)
	return JsonResponse(lst,safe=_A)
def whyus_create(request):
	context={};context[_J]=_AR;active_page=get_translated_active_page(_Ap);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_T
	if request.method==_I:
		form=WhyUsForm(request.POST)
		if form.is_valid():
			tmp=WhyUs.objects.filter(translations__title=request.POST.get(_F),site_id=site_id)
			if tmp:messages.info(request,mMsgBox.get(_Ax));context[_G]=WhyUsForm()
			else:post=WhyUs.objects.language(_B).create(title=request.POST.get(_F),description=request.POST.get(_A7),icon=request.POST.get(_P),status=request.POST.get(_N));post.set_current_language(_L);post.title=request.POST.get(_F);post.description=request.POST.get(_A7);post.icon=request.POST.get(_P);post.save();messages.info(request,mMsgBox.get(_a,request.POST.get(_F)));return redirect(reverse_lazy(_AR))
...
def whyus_update(request,uuid):
	...
	return render(request,template,context)
def whyus_delete(request,uuid):context={};site_id=get_site_id(request);data=WhyUs.objects.filter(site_id=site_id,uuid=uuid);post=get_object_or_404(data);tmp=post.title;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_AR))
class UserView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(UserView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_A3);context[_E]=active_page;menu=get_menu_caches(self.request,_D,self.site_id,active_page);context.update(menu);return context
...
def user_create(request):
	...
	if request.method==_I:
		...
		if form.is_valid():user=form.save(commit=_A);user.site_id=site_id;user.save();current_site=get_current_site(request);subject='Confirm Your Email Address';uid=urlsafe_base64_encode(force_bytes(user.pk));token=account_activation_token.make_token(user);message=render_to_string('account_activation_email.html',{_A3:user,'domain':current_site.domain,'uid':uid,'token':token});email_from=getattr(settings,'EMAIL_HOST_USER','noreply@gmail.com');send_email(email_from,'suratiwan03@gmail.com',subject,message);print('email send complete!');return redirect('account_activation_sent')
		else:messages.info(request,mMsgBox.get(_AV,request.POST.get(_AY)));context[_G]=CustomUserCreationForm(request.POST)
	else:messages.info(request,mMsgBox.get(_U));context[_G]=CustomUserCreationForm()
	return render(request,template,context)
def user_update(request,uuid):
	context={};context[_J]=_A3;active_page=get_translated_active_page(_A3);context[_E]=active_page;site_id=get_site_id(request);menu=get_menu_caches(request,_D,site_id,active_page);context.update(menu);template=get_template(site_id,is_frontend=_A)+_V;data=User.objects.filter(uuid=uuid);post=get_object_or_404(data)
	if request.method==_I:
		form=CustomUserChangeForm(request.POST,instance=post)
		if form.is_valid():obj=data.get();obj.name=request.POST.get(_H);obj.email=request.POST.get(_AY);obj.save();messages.info(request,mMsgBox.get(_W,request.POST.get(_H)));return redirect(reverse_lazy(_A3))
	else:messages.info(request,mMsgBox.get(_X));context[_G]=CustomUserChangeForm(instance=post)
	return render(request,template,context)
def user_delete(request,uuid):context={};site_id=get_site_id(request);data=User.objects.filter(uuid=uuid);post=get_object_or_404(data);tmp=post.name;post.delete();messages.info(request,mMsgBox.get(_Y,tmp));return redirect(reverse_lazy(_A3))
def get_hitcount_daily(request):
	A='created__date';lst=[];tgl=datetime.now();site_id=get_site_id(request);content_type_id=ContentType.objects.get(app_label=_Aq,model=_AQ);content_type_id=content_type_id.id if content_type_id else _C;hitcount_id=HitCount.objects.filter(content_type_id=content_type_id,object_pk=site_id).first();hitcount_id=hitcount_id.id if hitcount_id else _C;start_date=add_months(tgl,-3);start_date=datetime(start_date.year,start_date.month,1,0,0,0);print(_Ar,start_date);res=calendar.monthrange(tgl.year,tgl.month);day=res[1];end_date=datetime(tgl.year,tgl.month,day,23,59,59);print(_As,end_date);hit=Hit.objects.filter(hitcount_id=hitcount_id,created__range=[start_date,end_date]).values(A).annotate(count=Count(_B)).order_by(A);cat=[];val=[]
	for i in hit:tmp=[];dtime=i[A];cat.append(dtime.strftime(_At));val.append(i[_Au])
	lst.append(cat);lst.append(val);return lst
def get_hitcount_monthly(request):
	B='created__month';A='created__year';lst=[];tgl=datetime.now();site_id=get_site_id(request);content_type_id=ContentType.objects.get(app_label=_Aq,model=_AQ);content_type_id=content_type_id.id if content_type_id else _C;hitcount_id=HitCount.objects.filter(content_type_id=content_type_id,object_pk=site_id).first();hitcount_id=hitcount_id.id if hitcount_id else _C;start_date=add_months(tgl,-3);start_date=datetime(start_date.year,start_date.month,1,0,0,0);print(_Ar,start_date);res=calendar.monthrange(tgl.year,tgl.month);day=res[1];end_date=datetime(tgl.year,tgl.month,day,23,59,59);print(_As,end_date);hit=Hit.objects.filter(hitcount_id=hitcount_id,created__range=[start_date,end_date]).values(A,B).annotate(count=Count(_B)).order_by(A,B);print(hit);cat=[];val=[]
	for i in hit:tmp=[];dtime=i[B];dtime_year=i[A];cat.append(calendar.month_abbr[dtime]+' '+str(dtime_year));val.append(i[_Au])
	lst.append(cat);lst.append(val);return lst
def get_hitcount_weekly(request):
	lst=[];tgl=datetime.now();site_id=get_site_id(request);content_type_id=ContentType.objects.get(app_label=_Aq,model=_AQ);content_type_id=content_type_id.id if content_type_id else _C;hitcount_id=HitCount.objects.filter(content_type_id=content_type_id,object_pk=site_id).first();hitcount_id=hitcount_id.id if hitcount_id else _C;start_date=add_months(tgl,-3);start_date=datetime(start_date.year,start_date.month,1,0,0,0);print(_Ar,start_date);res=calendar.monthrange(tgl.year,tgl.month);day=res[1];end_date=datetime(tgl.year,tgl.month,day,23,59,59);print(_As,end_date);hit=Hit.objects.filter(hitcount_id=hitcount_id,created__range=[start_date,end_date]);week_begin,week_end=get_week_date(tgl.year,tgl.month,tgl.day);cat=[];val=[]
	for i in range(13):
		...
		if hit_2:
			for j in hit_2:cat.insert(0,week_begin_2.strftime(_At)+' - '+week_end_2.strftime(_At));val.insert(0,j[_Au])
	lst.append(cat);lst.append(val);return lst
