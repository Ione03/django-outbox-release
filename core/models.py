...
class ModelList(BaseAbstractModel):
	name=models.CharField(_(_C),max_length=50);description=models.CharField(_('desciption'),max_length=255);templates=models.ManyToManyField(Template,through='ModelListSetting');status=models.SmallIntegerField(choices=OptStatusDefault.choices,default=OptStatusDefault.OPTIONAL)
	class Meta:verbose_name=_('model list');verbose_name_plural=_('models list')
	def __str__(self):return self.name
class ModelListSetting(BaseAbstractModel):
	model_list=models.ForeignKey(ModelList,on_delete=models.CASCADE);template=models.ForeignKey(Template,on_delete=models.CASCADE);image_width=models.SmallIntegerField();image_height=models.SmallIntegerField()
	class Meta:verbose_name=_('model list setting');verbose_name_plural=_('model list settings')
