...
def service_exists(request):
	A=get_site_id_front(request);print('site id from service is exists',A);C=Service.objects.filter(site_id=A)
	for B in C:
		if B.is_demo:return'demo'
		else:return B.get_kind_display()
	return None
