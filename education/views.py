...
def get_statistic(site_id,is_cache=False):
	...
	context[tmp]=tmp_cache;tmp='hit_last_week';tmp_cache=cache.get(tmp,version=site_id)
	...
	context[tmp]=tmp_cache;tmp='hit_last_month';tmp_cache=cache.get(tmp,version=site_id)
	...
	context[tmp]=tmp_cache;start_date=tgl+datetime.timedelta(hours=-5);start_date=datetime.datetime(start_date.year,start_date.month,start_date.day,start_date.hour,0,0);end_date=datetime.datetime(tgl.year,tgl.month,tgl.day,tgl.hour,59,59);hit_online=Hit.objects.filter(hitcount_id=hitcount_id,created__range=(start_date,end_date)).values('ip').order_by('ip').distinct();context['hit_online']=hit_online.count()if hit_online else 1;hit_count=HitCount.objects.filter(object_pk=site_id,content_type_id=content_type_id);context['hit_all']=hit_count[0].hits if hit_count else 1;return context
def get_banner(site_id):subquery_foto=get_photo(_L);return Banner.objects.filter(site_id=site_id).annotate(file_path=subquery_foto)[:1]
...
def get_slideshow(site_id,lang):subquery_foto=get_photo(_D);return SlideShow.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_A)
...
def get_socialmedia(site_id):return SocialMedia.objects.filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).order_by(_A)
...
class IndexView(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		if not service:raise Http404(_N%(request.get_host(),_O))
	...
	def get_context_data(self,*args,**kwargs):context=super(IndexView,self).get_context_data(*(args),**kwargs);context[_P]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,True);context.update(statistic);context[_F]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_L]=get_banner(self.site_id);context[_C]=get_announcement(self.site_id,lang,4);context[_D]=get_slideshow(self.site_id,lang);context['dailyalert']=get_dailyalert(self.site_id,lang);context[_E]=get_greeting(self.site_id,lang);context[_H]=get_events(self.site_id,lang);context[_I]=get_photogallery(self.site_id,lang);context[_Q]=get_relatedlink(self.site_id,lang);context[_J]=get_news(self.site_id,lang);context[_R]=get_document(self.site_id,lang);context[_S]=get_socialmedia(self.site_id);context[_T]=get_base_url(self.request);return context
class DetailView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		B='kind';A='pages';context=super(DetailView,self).get_context_data(*(args),**kwargs);context[_P]=get_menu_group(self.site_id);slug=self.kwargs['slug']
		if not slug:raise Http404("Menu Group '%s' belum terdaftar, silahkan daftar di halaman <a href='%s'>admin</a>"%(self.request.get_host(),_K))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,True);context.update(statistic);context[_F]=get_logo(self.site_id);lang=get_active_language_choices()[0];kinds=[_C,_J,'article',_H,_R,_E,A,_D];kind=self.kwargs[B]
		...
		content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context['content_detail']=content_detail;hit_count=HitCount.objects.get_for_object(self.request,content_detail);hit_count_response=HitCountMixin.hit_count(self.request,hit_count);print(hit_count_response);context[_I]=get_photogallery(self.site_id,lang);context[_Q]=get_relatedlink(self.site_id,lang);context[_S]=get_socialmedia(self.site_id);context[_T]=get_base_url(self.request);return context