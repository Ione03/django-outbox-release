...
_k='latest_kind'
_j='categories_list'
_i="Menu Group '%s' belum terdaftar, silahkan daftar di halaman <a href='%s'>admin</a>"
_h='product'
_g='banner'
_f='document'
_e='/dashboard'
...
from .models import *
def get_calendar_ajax(request,service_type,year,month):A='get_calendar_ajax';print(A,request);print(A,year,month);res=sync_calendar_all(request,year,month);return JsonResponse(res,safe=_V)
...
def get_statistic(site_id,is_cache=_V):
...
def get_banner(site_id):subquery_foto=get_photo(_g);return Banner.objects.filter(site_id=site_id).annotate(file_path=subquery_foto)[:5]
...
def get_slideshow(site_id,lang):subquery_foto=get_photo(_L);obj=SlideShow.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B)[:10];obj=list(obj);random.shuffle(obj);return obj
def get_fasilities(site_id,lang,exclude_id=[],is_header_text=_A,is_shuffle=_A):
	subquery_foto=get_photo(_N)
	if is_header_text is _A:print('isheadertext1',is_header_text);obj=Fasilities.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).annotate(file_path=subquery_foto).order_by(_E,_I)[:10]
	else:print('isheadertext2',is_header_text);obj=Fasilities.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).exclude(is_header_text=_D).annotate(file_path=subquery_foto).order_by(_E,_I)[:10]
	obj=list(obj)
	if is_shuffle:random.shuffle(obj)
	return obj
def get_offers(site_id,lang,exclude_id=[],is_header_text=_A,is_shuffle=_A):
	subquery_foto=get_photo(_O)
	if is_header_text is _A:obj=Offers.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).annotate(file_path=subquery_foto).order_by(_E,_I)[:10]
	else:obj=Offers.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).exclude(id__in=exclude_id).exclude(is_header_text=_D).annotate(file_path=subquery_foto).order_by(_E,_I)[:10]
	return obj
...
def get_howitworks(site_id,lang):return HowItWorks.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).order_by(_E,_I)[:10]
def get_product(site_id,lang):subquery_foto=get_photo(_h);return Product.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_E,_I)[:10]
def get_greeting(site_id,lang):subquery_foto=get_photo(_J);return Greeting.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B)[:1]
def get_events(site_id,lang):subquery_foto=get_photo(_W);return Events.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B)[:7]
def get_photogallery(site_id,lang):subquery_foto=get_photo(_F);return PhotoGallery.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_E,_I)[:10]
def get_videogallery(site_id,lang):subquery_foto=get_photo(_X);return VideoGallery.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B)[:6]
...
def get_news(site_id,lang):subquery_foto=get_photo(_G);return News.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_B)[:7]
def get_article(site_id,lang):subquery_foto=get_photo(_Y);return Article.objects.language(lang).filter(site_id=site_id,status=OptStatusPublish.PUBLISHED).annotate(file_path=subquery_foto).order_by(_E,_B)[:6]
...
def get_categories_list(site_id,lang,max_data,model):
	B='count';A='categories_id';subquery=Subquery(Categories.objects.translated(lang).filter(id=OuterRef(A)).values('translations__name')[:1]);subquery_slug=Subquery(Categories.objects.translated(lang).filter(id=OuterRef(A)).values(_Z)[:1]);categories_list=[];obj=model.objects.filter(site_id=site_id).values(A).annotate(count=Count(A)).annotate(name=subquery).annotate(slug=subquery_slug).order_by(A)[:max_data]
	if obj:
		all=0
		for i in obj:all+=i[B]
		categories_list.append(obj[0]);categories_all={A:0,B:all,'name':'All',_Z:'all'};categories_list.insert(0,categories_all);return categories_list
...
def get_content_detail(site_id,lang,model,kind,slug):
	...
	else:raise Http404(_a)
def get_content_list(site_id,lang,model,kind,slug):
	A='-created_at'
	if not slug:raise Http404(_a)
	subquery_foto=get_photo(kind)
...
class IndexView(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		if not service_type:raise Http404("service belum terdaftar, silahkan daftar di halaman <a href='%s'>admin</a>"%_H)
		site=_A
		if service_type=='demo':
			shortuuid=kwargs.get('shortuuid','')
			if not shortuuid:raise Http404("UUID belum terdaftar, silahkan daftar di halaman <a href='%s'>admin</a>"%_H)
			if shortuuid:
				...
				else:raise Http404("short UUID belum terdaftar di agency, silahkan daftar di halaman <a href='%s'>admin</a>"%_H)
			...
			if not service:raise Http404(_P%(request.get_host(),_H))
	...
	def get_context_data(self,*args,**kwargs):context=super(IndexView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,1);context.update(menu);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_D);context.update(statistic);context[_K]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_g]=get_banner(self.site_id);context[_C]=get_announcement(self.site_id,lang,4);context[_L]=get_slideshow(self.site_id,lang);context['dailyalert']=get_dailyalert(self.site_id,lang);context['howitworks']=get_howitworks(self.site_id,lang);context[_h]=get_product(self.site_id,lang);context['whyus']=get_whyus(self.site_id,lang);context[_N]=get_fasilities(self.site_id,lang);context[_O]=get_offers(self.site_id,lang);context[_J]=get_greeting(self.site_id,lang);context[_W]=get_events(self.site_id,lang);context[_F]=get_photogallery(self.site_id,lang);context[_X]=get_videogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_G]=get_news(self.site_id,lang);context[_Y]=get_article(self.site_id,lang);context[_f]=get_document(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context['location']=get_location(self.site_id,lang);context[_U]=get_base_url(self.request);return context
class DetailView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(DetailView,self).get_context_data(*(args),**kwargs);print('enter detail');active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,1);context.update(menu);context[_b]=get_menu_group(self.site_id);slug=self.kwargs[_Z]
		if not slug:raise Http404(_i%(self.request.get_host(),_e))
		print('after slug');agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_D);context.update(statistic);context[_K]=get_logo(self.site_id);lang=get_active_language_choices()[0];kinds=[_C,_G,_Y,_W,_f,_J,_c,_L,_N,_O];kind=self.kwargs[_d]
		if kind in kinds:context[_d]=kind
		else:raise Http404(_a)
		context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_M,kind);print('model=',model)
		if kind not in[_c,_J,_L,_N,_O]:context[_j]=get_categories_list(self.site_id,lang,10,model);context[_k]=get_latest_model(self.site_id,lang,3,model,kind,slug);context[_l]=get_related_model(self.site_id,lang,5,model,kind,slug)
		print(_m,model,kind);content_detail=get_content_detail(self.site_id,lang,model,kind,slug);context['content_detail']=content_detail;hit_count=HitCount.objects.get_for_object(content_detail);hit_count_response=HitCountMixin.hit_count(self.request,hit_count);context[_F]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_N]=get_fasilities(self.site_id,lang,[content_detail.id],_V,_D);context[_O]=get_offers(self.site_id,lang,[content_detail.id],_V,_D);context[_T]=get_socialmedia(self.site_id);context[_U]=get_base_url(self.request);return context
class ListView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):
		context=super(ListView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,1);context.update(menu);context[_b]=get_menu_group(self.site_id);slug=self.kwargs[_Z]
		if not slug:raise Http404(_i%(self.request.get_host(),_e))
		agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_D);context.update(statistic);context[_K]=get_logo(self.site_id);lang=get_active_language_choices()[0];kinds=[_C,_G,_Y,_W,_f,_J,_c,_L,_X,_F];kind=self.kwargs[_d]
		if kind in kinds:context[_d]=kind
		else:raise Http404(_a)
		print('Kind = ',kind);context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_M,kind)
		if kind not in[_c,_J,_L,_X,_F]:context[_j]=get_categories_list(self.site_id,lang,10,model);context[_k]=get_latest_model(self.site_id,lang,3,model,kind,slug);context[_l]=get_related_model(self.site_id,lang,5,model,kind,slug)
		print(_m,model,kind);content_list=get_content_list(self.site_id,lang,model,kind,slug)
		...
		context[_F]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_U]=get_base_url(self.request);return context
class DescriptionView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(DescriptionView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,1);context.update(menu);context[_b]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_D);context.update(statistic);context[_K]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_C]=get_announcement(self.site_id,lang,6);model=apps.get_model(_M,_C);context[_n]=get_latest_model(self.site_id,lang,3,model,_C);model=apps.get_model(_M,_G);context[_o]=get_latest_model(self.site_id,lang,3,model,_G);context[_F]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_U]=get_base_url(self.request);return context
class GreetingView(TemplateView):
	...
	def get_context_data(self,*args,**kwargs):context=super(GreetingView,self).get_context_data(*(args),**kwargs);active_page=get_translated_active_page(_Q);menu=get_menu_caches(self.request,_R,self.site_id,active_page,1);context.update(menu);context[_b]=get_menu_group(self.site_id);agency=get_agency_info(self.site_id);context.update(agency);statistic=get_statistic(self.site_id,_D);context.update(statistic);context[_K]=get_logo(self.site_id);lang=get_active_language_choices()[0];context[_C]=get_announcement(self.site_id,lang,6);context[_J]=get_greeting(self.site_id,lang);model=apps.get_model(_M,_C);context[_n]=get_latest_model(self.site_id,lang,3,model,_C);model=apps.get_model(_M,_G);context[_o]=get_latest_model(self.site_id,lang,3,model,_G);context[_F]=get_photogallery(self.site_id,lang);context[_S]=get_relatedlink(self.site_id,lang);context[_T]=get_socialmedia(self.site_id);context[_U]=get_base_url(self.request);return context