...
class Logo(BaseAbstractModel):
	...
	def __unicode__(A):return A.name
	def save(A,*B,**C):A.slug=slugify(A.name)+_B+str(A.site_id);A.site_id=get_site_id(exposed_request);super(Logo,A).save(*(B),**C)
...
class Tags(BaseAbstractModel,TranslatableModel):
	...
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.name)+_B+str(A.site_id);A.site_id=get_site_id(exposed_request);super(Tags,A).save(*(B),**C)
class Categories(BaseAbstractModel,TranslatableModel):
	site=models.ForeignKey(Site,on_delete=models.CASCADE);translations=TranslatedFields(name=models.CharField(_('categories name'),max_length=50));slug=models.SlugField(max_length=50,default='',unique=_A,blank=_A,editable=_D)
	class Meta:verbose_name=_('category');verbose_name_plural=_('categories')
	def __unicode__(A):return A.name
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.name)+_B+str(A.site_id);A.site_id=get_site_id(exposed_request);super(Categories,A).save(*(B),**C)
...
class Announcement(BaseAbstractModel,BaseContentModel,TranslatableModel):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=500)),content=encrypt(RichTextUploadingField(_(_G),blank=_A,null=_A)));categories=models.ForeignKey(Categories,on_delete=models.PROTECT)
	class Meta:verbose_name=_('announcement');verbose_name_plural=_('announcements')
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.title)+_B+str(A.id);A.site_id=get_site_id(exposed_request);A.admin_id=exposed_request.user.id;super(Announcement,A).save(*(B),**C)
class News(BaseAbstractModel,BaseContentModel,TranslatableModel):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=500)),content=encrypt(RichTextUploadingField(_(_G),blank=_A,null=_A)));categories=models.ForeignKey(Categories,on_delete=models.PROTECT)
	class Meta:verbose_name=_('news');verbose_name_plural=_('news')
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.title)+_B+str(A.id);A.site_id=get_site_id(exposed_request);A.admin_id=exposed_request.user.id;super(News,A).save(*(B),**C)
class Article(BaseAbstractModel,BaseContentModel,TranslatableModel):
	...
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.title)+_B+str(A.id);A.site_id=get_site_id(exposed_request);A.admin_id=exposed_request.user.id;super(Article,A).save(*(B),**C)
class Events(BaseAbstractModel,BaseContentModel,TranslatableModel):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=500)),content=encrypt(RichTextUploadingField(_(_G),blank=_A,null=_A)),location=encrypt(models.CharField(_('location'),max_length=255,null=_A,blank=_A)));categories=models.ForeignKey(Categories,on_delete=models.PROTECT);date=models.DateField(_('date'));time=models.TimeField(_('time'))
	class Meta:verbose_name=_('event');verbose_name_plural=_('events')
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.title)+_B+str(A.id);A.site_id=get_site_id(exposed_request);A.admin_id=exposed_request.user.id;super(Events,A).save(*(B),**C)
...
class DailyAlert(BaseAbstractModel,TranslatableModel):
	...
	def __unicode__(A):return A.alert
...
class Pages(BaseAbstractModel,BaseContentModel,TranslatableModel):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=500)),content=encrypt(RichTextUploadingField(_(_G),blank=_A,null=_A)));menu=models.ForeignKey(Menu,on_delete=models.PROTECT,verbose_name='Access From Menu')
	class Meta:verbose_name=_('page');verbose_name_plural=_('pages')
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.title)+_B+str(A.id);A.site_id=get_site_id(exposed_request);A.admin_id=exposed_request.user.id;super(Pages,A).save(*(B),**C)
class SocialMedia(BaseAbstractModel):
	...
	def __unicode__(A):return '{} - {}'.format(A.site.name,A.kind)
...
class PhotoGallery(BaseAbstractModel,BaseGalleryModel,TranslatableModel):
	translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=500)));photo=GenericRelation(Photo,verbose_name=_(_I))
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.title)+_B+str(A.id);A.site_id=get_site_id(exposed_request);A.admin_id=exposed_request.user.id;super(PhotoGallery,A).save(*(B),**C)
class VideoGallery(BaseAbstractModel,BaseGalleryModel,TranslatableModel):
	...
	def __unicode__(A):return A.title
	def save(A,*B,**C):A.slug=A.get_current_language()+_E+slugify(A.title)+_B+str(A.id);A.site_id=get_site_id(exposed_request);A.admin_id=exposed_request.user.id;super(VideoGallery,A).save(*(B),**C)
...
class Popup(BaseAbstractModel,TranslatableModel):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_(_F));admin=models.ForeignKey(User,on_delete=models.PROTECT);translations=TranslatedFields(title=encrypt(models.CharField(_(_C),max_length=500)));link=encrypt(models.URLField(_(_H),max_length=255,null=_A,blank=_A));photo=GenericRelation(Photo,verbose_name=_(_I));status=models.SmallIntegerField(choices=OptStatusPublish.choices,default=OptStatusPublish.PUBLISHED)
	def __unicode__(A):return A.title
